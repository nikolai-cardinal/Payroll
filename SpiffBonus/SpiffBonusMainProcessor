...
function processSpiffAndBonus(techSheet, technicianName, spiffBonusSheet, ratesSheet, triggerRow, triggerCol, suppressPopup = false) {
  ...
  try {
    ...
    if (typeof isTechnicianClass1 === 'function' && isTechnicianClass1(technicianName, ratesSheet)) {
      ...
      if (!suppressPopup) {
        ui.alert(`Skipped Spiff/Bonus processing for ${technicianName} (Class 1 technician)`);
      }
      return;
    }
    ...
    // --- 7. Clear existing Spiff rows BEFORE updating summary ---
...
function processSpiffBonusAll() {
  ...
  for (var i = ratesMapping.dataStartRow - 1; i < ratesData.length; i++) { // Adjust index for 0-based array
    ...
    if (techName && typeof techName === 'string' && techName.trim() !== "" && sheetNames.indexOf(techName) !== -1) {
      // Check if tech is Class 1 (column C)
      var positionValue = row[2]; // Column C (index 2)
      var isClass1 = positionValue && typeof positionValue === 'string' && 
                     positionValue.toString().toLowerCase().includes('class 1');
      
      if (isClass1) {
        console.log(`Skipping Class 1 technician: ${techName}`);
        skippedClass1.push(techName);
        continue;
      }
  ...
  if (techsToProcess.length === 0) {
    var message = 'No technicians with sheets found for Spiff/Bonus processing.';
    if (skippedClass1.length > 0) {
      message += `\n\nNote: Skipped ${skippedClass1.length} Class 1 technicians: ${skippedClass1.join(', ')}`;
    }
    ui.alert(message);
    return;
  }
  ...
  // Add Class 1 section if any techs were skipped
  if (skippedClass1.length > 0) {
    summaryMessage.push("\nSKIPPED (CLASS 1 TECHNICIANS):");
    summaryMessage.push("• Total skipped: " + skippedClass1.length);
    summaryMessage.push("• Technicians: " + skippedClass1.join(", "));
  }
