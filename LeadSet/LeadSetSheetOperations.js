...
function getLeadDataForTechnician(leadSetSheet, technicianName, dateRange) {
  try {
    if (!leadSetSheet) {
      console.error("Error: Lead Set sheet is null or undefined");
      return [];
    }
    
    if (!technicianName) {
      console.error("Error: Technician name is required");
      return [];
    }
    
    const data = leadSetSheet.getDataRange().getValues();
    if (!data || data.length <= 1) {
      console.log("Lead Set sheet is empty or contains only headers");
      return [];
    }
    
    const headers = data[0];
    console.log(`Processing Lead Set sheet with ${data.length-1} entries`);
    
    // We're going to use the actual column positions based on the Lead Set sheet
    // Column A (0): Invoice ID
    // Column B (1): Completion Date
    // Column C (2): Customer Name
    // Column D (3): Business Unit
    // Column E (4): Job Total Revenue
    // Column F (5): Balance
    // Column G (6): Lead Generated By
    // Column H (7): Sold By
    // Column I (8): Assigned Technicians
    
    // Filter data for the specified technician in Column G (Lead Generated By)
    const technicianData = [];
    for (let i = 1; i < data.length; i++) {
      const row = data[i];
      
      // Skip rows with no data in key columns
      if (!row[2] || !row[4] || !row[6]) continue;
      
      // Check if this technician is in the "Lead Generated By" column (G - index 6)
      if (row[6] === technicianName) {
        const completionDate = row[1];
        
        // Apply date filter if provided
        if (dateRange && dateRange.startDate && dateRange.endDate) {
          if (!(completionDate instanceof Date)) {
            // Try to convert to date if it's not already a date
            const parsedDate = new Date(completionDate);
            if (isNaN(parsedDate.getTime())) {
              console.log(`Skipping row ${i+1} - Invalid completion date: ${completionDate}`);
              continue; // Skip entries with invalid dates
            }
          }
          
          // Skip if date is outside the specified range
          if (completionDate < dateRange.startDate || completionDate > dateRange.endDate) {
            continue;
          }
        }
        
        technicianData.push({
          customer: row[2],          // Column C - Customer Name
          businessUnit: row[3],      // Column D - Business Unit
          completionDate: row[1],    // Column B - Completion Date
          revenue: row[4] || 0,      // Column E - Job Total Revenue
          notes: '',                 // No specific notes column
        });
      }
    }
